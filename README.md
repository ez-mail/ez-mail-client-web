...
이때 현재 마우스 커서가 있는 블록을 기준으로 해당 블록 높이의 반보다 커서가 위에 있으면 해당 블록 위에 들어가는 위치가 표시되고, 그 블록의 높이의 반보다 커서가 아래에 있으면 해당 블록 아래에 들어가는 위치가 표시되도록 코드를 작성해서 사용자가 직관적으로 블록이 놓일 위치를 알 수 있도록 했습니다.
...
제어 컴포넌트로 만들어도 문제고 비제어 컴포넌트로 만들어도 문제인데 어떻게 해결해야 할지 난감했습니다. 그러다 '**둘 중 하나로 규정해서 문제가 됐으니 제어 컴포넌트이면서 비제어 컴포넌트로 만들면 해결되지 않을까?**' 하는 생각이 번뜩 들었습니다. 아래 그림처럼 컴포넌트를 2중으로 사용해서 내부는 비제어 컴포넌트로 외부는 제어 컴포넌트로 사용해서 내부 컴포넌트의 데이터를 다룬다면 문제가 해결될 것 같다는 생각이 들었습니다.
...
마지막으로 [mailtrap](https://mailtrap.io/)에서 최종적으로 발송된 이메일 템플릿의 호환성을 테스트한 결과 96%라는 수치로 시중의 웬만한 이메일 클라이언트는 모두 호환되는 이메일 템플릿을 생성할 수 있었습니다.
